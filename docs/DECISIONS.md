...
**Decision 3 — CI now runs on `main`, and the backend suite runs at all.** `ruff` and `black` had been failing on `main` for several commits with nothing to report it: `ci.yml` triggered only on pull requests and the old `rebuild/phase-0` branch, while `deploy.yml` fires on every push to `main`. Work has been reaching the live boxes with no gate having an opinion about it.

Turning the trigger on found something worse on the very first run. **The backend test suite had never once executed on a runner.** Every test errored at setup with `ValueError: invalid literal for int() with base 10: 'None'`: `pgserver` listens on TCP on Windows but on a Unix domain socket on Linux, where `get_uri()` carries no hostname and no port — the socket directory arrives as a `?host=` query parameter — and `conftest.py` formatted those `None`s straight into a connection string. Locally it always passed, because local is Windows. So the cross-tenant isolation suite that spec G.1 makes a *build-failing requirement* had been failing to run, silently, for the entire life of the project. `postgres_urls` handles both transports now.

That is the whole argument for this decision in one incident: a gate that never runs is indistinguishable from a gate that always passes, and both are indistinguishable from having no gate — right up until something reaches production.

CI still does **not** block the deploy: both workflows trigger independently on the same push, so a red run and a green deploy can coexist. Gating deploy on CI (`workflow_run`, or a `needs:` prerequisite) is the right follow-up, deliberately not bundled into a hotfix — getting it wrong breaks the ability to ship at all.

**Decision 4 — the query cache no longer survives a change of who is signed in.** Same family as the branding bug, found while fixing it: the React Query cache outlives a sign-out, so signing into a second business on the same device served the first one's cached answers — business name, day status, everything — until each query happened to refetch. All of it is tenant-scoped, and showing a shopkeeper another shop's figures even briefly is precisely the failure this codebase makes build-failing on the server. `clearCacheOnAuthChange` drops the whole cache on any change of sign-in state, wholesale rather than key by key, so a query added later is covered without anyone remembering it exists — the same reasoning that put error surfacing on the cache rather than in each of forty hooks.

**Decision 5 — the session probe asks only when there is something to ask about.** Restoring a session means calling `GET /users/me` on mount, which for a first-time visitor is a guaranteed 401 and a console error on the login page of every first visit. A successful sign-in now leaves a readable `operatoros_session_hint` in localStorage and the probe runs only when it is present; a 401 clears it, so an expired session stops re-asking on every load, while a network error keeps it so the next load retries. The marker authorises nothing — the server still decides, and the flow still fails closed. It only distinguishes "might be signed in, worth asking" from "definitely never signed in here", which is not something httpOnly cookies can tell you from JS.
